...
1. Собственно, добавляем пользователя okfilm, пароль ему не задаём, доступ только по ssh:  
   `$ sudo adduser okfilm`    
...
1. Собственно, добавляем пользователя devops без пароля, доступ только по ssh:  
   `$ sudo adduser devops`  
