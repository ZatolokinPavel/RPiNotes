...
#### Удаление таблицы из базы PostgreSQL  
Вдруг код изменился, и нужно, не заморачиваясь, удалить таблицу, чтобы пересоздать её заново.  
В принципе, ничего особенного, просто заметка для быстрого доступа.  
```
sudo -u postgres psql
\list
\c database_name
