...
3. Выключаем локализацию `en_GB.UTF-8 UTF-8`  
   выбираем локализацию `ru_RU.UTF-8 UTF-8`  
   Включение и выключение производится пробелом.
4. Далее нам предложат выбрать локализацию по умолчанию. Выбираем `ru_RU.UTF-8`

После выставления всех настроек нажимаем **Finish**. Raspberry предложит перезагрузиться. Соглашаемся. После перезагрузки логинимся в систему, и понимаем, что вместо русских символов в консоли печатаются квадратики. Чтобы этого не было выполняем команду:  
