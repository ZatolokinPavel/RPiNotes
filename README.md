...
5. [Настроить SSH доступ и локальную сеть.](SSH.md)
...
9. [Настроить разделы на флешке](Disk%20partitioning.md)
10. [Настроить пользователей системы.](Users.md)
11. [Установить и настроить Samba для создания общей папки.](Samba.md)
12. (нет) Добавить репозиторий **bullseye**. [(см. ниже)](#Добавление-репозитория-bullseye)
13. [Настроить часы реального времени.](RTC.md)
14. [Настроить watchdog.](Watchdog.md)
15. (sh) Отключить Wi-Fi и Bluetooth [(см. ниже)](#Отключение-Wi-Fi-и-Bluetooth)
16. (sh) Применить дополнительные настройки [(см. ниже)](#Дополнительные-настройки)
17. (sh) [Установить и настроить Nginx.](Nginx.md)
18. (sh) [Настроить SSL для Nginx (https).](SSL%20(https).md)
19. [Вручную получить сертификаты SSL (https), если выполнялся скрипт общей настройки.](SSL%20(https).md#Итого)
20. (sh) [Скачать сайт okfilm.com.ua и подключить его к Nginx.](https://github.com/ZatolokinPavel/okfilm_2018)
...
Перед запуском на компе открыть файл `/boot/cmdline.txt` и удалить оттуда строчку `init=/usr/lib/raspi-config/init_resize.sh`. Этот файл в винде виден прямо в корне диска `boot` флешки. Удаление этой строчки отключит автоматическое расширение файловой системы root на весь размер флешки. Так что появится возможность создать отдельный раздел на флешке для файлообменника "OkFILM диск".  
Запустить на малине. Логин/пароль по умолчанию `pi` `raspberry`.  
...
* **5 Localisation Options** – здесь нужно будет установить локаль, часовой пояс и настроить клавиатуру. Об этом отдельно.
