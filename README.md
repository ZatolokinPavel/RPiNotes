...
## План
1. Установить операционную систему Raspberry Pi OS Lite (64-bit). [(обязательно см. ниже)](#Установка-операционной-системы-Raspberry-Pi-OS-Lite-64-bit)
...
5. Подключиться по SSH через логин+пароль (уже можно).
6. Выполнить [скрипт настройки](#Скрипт-настройки).  
   Все последующие пункты с пометкой (sh) не выполнять - они есть в скрипте.
7. [Настроить SSH доступ и локальную сеть.](SSH.md)
8. (sh) Обновить всё что только можно [(см. ниже)](#Обновление)
9. (sh) Установка необходимого минимума программ [(см. ниже)](#Установка-необходимого-минимума-программ)
10. (sh) [Настроить разделы на карте памяти](Disk%20partitioning.md)
11. (sh) [Смонтировать дополнительный раздел](Монтирование%20диска%20или%20флешки.md)
12. Проверить, не сменился ли UUID внешних флешек [(см. тут)](Монтирование%20диска%20или%20флешки.md#проверка-uuid).
13. (sh) [Настроить пользователей системы.](Users.md)
14. [Установить и настроить Samba для создания общей папки.](Samba.md)
15. (нет) Добавить репозиторий **Trixie**. [(см. ниже)](#Добавление-репозитория-Trixie)
16. [Настроить часы реального времени.](RTC.md)
17. [Настроить watchdog.](Watchdog.md)
18. (sh) Отключить Wi-Fi и Bluetooth [(см. ниже)](#Отключение-Wi-Fi-и-Bluetooth)
19. (sh) Применить дополнительные настройки [(см. ниже)](#Дополнительные-настройки)
20. (sh) [Установить и настроить Nginx.](Nginx.md)
21. (sh) [Настроить SSL для Nginx (https).](SSL%20(https).md)
22. [Вручную получить сертификаты SSL (https), если выполнялся скрипт общей настройки.](SSL%20(https).md#Итого)
23. (sh) [Скачать сайт okfilm.com.ua и подключить его к Nginx.](https://github.com/ZatolokinPavel/okfilm_2018)
...
31. Если надо, установить [Apache, PHP и MySQL](PHP.md)

### Установка операционной системы Raspberry Pi OS Lite (64-bit)
Скачать с [офф. сайта](https://www.raspberrypi.com/software/operating-systems/#raspberry-pi-os-64-bit) и записать на флешку с помощью **Win32DiskImager**.  
Пытался через Raspberry Pi Imager, но он ругается на несовпадение SHA256.  

Перед запуском нужно отключить расширение файловой системы root на весь размер флешки. Чтобы появилась возможность создать отдельный раздел на флешке `userdisk` и не переполнять системный диск. Например, для файлообменника "OkFILM диск". Есть два способа:  
_Лучше:_  
...
Запустить на малине.  
При первом запуске сразу предложит выбрать раскладку клавиатуры, и задать имя пользователя и пароль.  

Обязательно удалить созданный ранее вспомогательный раздел! Иначе скрипт не разметит правильно разделы. (Как удалить - описано перед применением скрипта).
...
Также подготовить публичные ssh-ключи для пользователей `pi`, `okfilm`, `devops` и GitHub Personal access token для скачивания приватных репозиториев.  
