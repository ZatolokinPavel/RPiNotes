...
#MaxAuthTries 6
#MaxSessions 10
AllowUsers pi okfilm devops # по ssh разрешено заходить ТОЛЬКО этим пользователям
...
`ssh-keygen -t ed25519 -C "user@server"` - новый, быстрый, более стойкий чем RSA 2048 bit  
`ssh-keygen -t rsa -b 4096 -C "user@server"` - старый, медленный  
...
### Устаревший сертификат RSA
Сертификаты RSA устарели. Они построены на факторизации больших полупростых чисел. Медленные и большого размера.  
Сертификаты ED25519 современные. Построены на эллиптических кривых, быстрые, маленького размера. По стойкости лучше, чем RSA 2048 bit.  
Временно включить RSA можно добавив команту `PubkeyAcceptedKeyTypes +ssh-rsa` в конфиг.  
Но нужно переписать на что-то новое.  
